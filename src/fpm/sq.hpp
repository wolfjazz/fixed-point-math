...
public:
    ...
    static constexpr
    Sq fromReal() { return Sq( fpm::scaled<Sq>(realValue) ); }
...
#   endif
    constexpr
    TargetT toReal() const noexcept { return fpm::real<Sq, TargetT>(this->value); }
...
private:
    ...
    struct ShiftL {
        ...
        static constexpr scaling_t f = Sq::f;
        static constexpr double realVMin = fpm::real<Sq>(static_cast<interm_t<base_t>>(Sq::vMin) << ic);
        static constexpr double realVMax = fpm::real<Sq>(static_cast<interm_t<base_t>>(Sq::vMax) << ic);
        static constexpr base_t op(typename Sq::base_t lv, std::integral_constant<T, ic>) noexcept {
            return lv << ic;
        }
    };
    ...
    struct CTClamp {
        ...
        static constexpr base_t lo = fpm::scaled<Sq>(realVMin);
        static constexpr base_t hi = fpm::scaled<Sq>(realVMax);
