...
def print_sq_types():
    for sq in sqTypeList:
        ...
        for f, fStr in f_range:
            print("template< double lower = fpm::realMin<%8s, %3s>(), double upper = fpm::realMax<%8s, %3s>() > using %8s = sq<%8s, %3s, lower, upper>;" % (sq.type, f, sq.type, f, sq.short + "sq" + fStr, sq.type, f))
