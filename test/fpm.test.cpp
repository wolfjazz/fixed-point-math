...
TEST_F(V2STest, v2s__constexpr_double_positiveF__double) {
    constexpr scaling_t to = 7;
    auto result = v2s<to,double>(-564.1);

    constexpr double expectedResult = -72204.8;
    ASSERT_NEAR(expectedResult, result, 0.1);
}

TEST_F(V2STest, v2s__constexpr_double_negativeF__double) {
    constexpr scaling_t to = -6;
    auto result = v2s<to,double>(-29876.1);

    constexpr double expectedResult = -466.8;
    ASSERT_NEAR(expectedResult, result, 0.1);
}

TEST_F(V2STest, v2s__constexpr_double_f0__same_double) {
    constexpr scaling_t to = 0;
    constexpr double real = -564.1;
    auto result = v2s<to,double>(real);

    ASSERT_NEAR(real, result, 0.001);
}

TEST_F(V2STest, v2s__constexpr_signed_positiveF__int) {
    constexpr scaling_t to = 9;
    constexpr int8_t value = 4;
    auto result = v2s<to,int>(value);

    constexpr int expectedResult = 2048;
    ASSERT_EQ(expectedResult, result);
}

TEST_F(V2STest, v2s__constexpr_signed_negativeF__int) {
    constexpr scaling_t to = -6;
    constexpr int16_t value = -512;
    auto result = v2s<to,int>(value);
...
TEST_F(V2STest, v2s__constexpr_signed_f0__int) {
    constexpr scaling_t to = 0;
    constexpr int16_t value = -512;
    auto result = v2s<to,int>(value);
...
TEST_F(V2STest, v2s__constexpr_small_signed_min_value_negativeF__larger_unsigned) {
    constexpr scaling_t to = -6;
    constexpr int16_t value = INT16_MIN;
    auto result = v2s<to,uint32_t>(value);  // -32768 / 2^6 = -512

    constexpr uint32_t expectedResult = 4294966784;  // -512, interpreted as u32
    ASSERT_EQ(expectedResult, result);
}

TEST_F(V2STest, v2s__constexpr_small_unsigned_max_value_negativeF__larger_signed) {
    constexpr scaling_t to = -6;
    constexpr uint16_t value = UINT16_MAX;
    auto result = v2s<to,int32_t>(value);

    constexpr int32_t expectedResult = 1023;
    ASSERT_EQ(expectedResult, result);
}

TEST_F(V2STest, v2s__constexpr_signed_negativeF__int_symmetric_output) {
    constexpr scaling_t to = -4;
    constexpr int16_t value = 514;
    auto resultP = v2s<to,int>(+value);
    auto resultN = v2s<to,int>(-value);
...
TEST_F(V2STest, v2s__constexpr_large_F__double_output) {
    constexpr double realMin = std::numeric_limits<int16_t>().min();
    constexpr double realMax = std::numeric_limits<int16_t>().max();
    auto resultN = v2s<detail::MAX_F-std::numeric_limits<int16_t>::digits,double>(realMin);  // f: 53-15=38
    auto resultP = v2s<detail::MAX_F-std::numeric_limits<int16_t>::digits,double>(realMax);
